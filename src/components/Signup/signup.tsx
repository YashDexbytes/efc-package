...
import AlertComponent from "../alerts/AlertComponent"; // Assuming you have this component
import {
  GoogleReCaptchaProvider,
  useGoogleReCaptcha,
} from "react-google-recaptcha-v3";
import { signupUser } from "@/apiService/auth";
...
const SignupForm = () => {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      setAlertMessage({
        message: (err as Error).message || "An error occurred during signup",
        type: "error",
      });
    } finally {
      setSubmitting(false);
  ...
  return (
    <div className="signup-page h-screen overflow-auto rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      {alertMessage && (
        <AlertComponent
          message={alertMessage.message}
          isError={alertMessage.type === "error"}
          duration={parseInt(
      ...
      <div className="signup-part flex h-full flex-wrap items-center">
        ...
        <div className="w-full border-stroke dark:border-strokedark md:w-1/2 md:border-l-2">
          <div className="w-full p-4 sm:p-6.5 xl:p-7.5">
            <div className="flex justify-center">
              <Link className="mb-5.5 inline-block md:hidden" href="/">
                <Image
                  ...
                  <div className="mb-3 flex flex-col space-y-3 sm:flex-row sm:space-x-3 sm:space-y-0">
                    <div className="flex-1">
                      ...
                      <Field
                        ...
                        className="w-full rounded border border-stroke bg-transparent py-3 pl-4 pr-8 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                      />
                      <ErrorMessage
                        name="firstName"
                        component="p"
                        className="input-error mt-1 text-sm"
                      />
                    </div>
                    <div className="flex-1">
                      <label className="mb-2 block font-medium text-black dark:text-white">
                        ...
                        {() => (
                          <PhoneInput
                            ...
                            onChange={(phone, country: any) => {
                              country.dialCode = country.dialCode ?? "91";
                        ...
                        <div className="dob w-1/2">
                          <Select
                            ...
                            onChange={(selectedOption) => {
                              if (selectedOption) {
                                setFieldValue("dob.day", selectedOption.value);
                              } else {
                                setFieldValue("dob.day", "");
                              }
                            }}
                            placeholder="DD"
                            isClearable
